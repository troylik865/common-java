			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						maxlength : $th.attr("maxlength") || "10",//2013-07-29 jiangb 增加长度限制属性
						min : $th.attr("min") || "",//2013-08-01 jiangb 增加最小值与最大值限制
						max : $th.attr("max") || ""
					};
					fields.push(field);
				});
				...
				$tbody.find("a.btnDel").click(function(){
					var $btnDel = $(this);
					//2013-07-29 jiangb 修改  [href="javascript:"] --> [href="javascript:void(0)"]
					//IE运行后点击删除行出现错误，但是FireFox却正常可以删除,经判断，应该是Jquery兼容问题
				...
				switch(field.type){
					...
					default :
						html = '<input type="text" name="' + field.name
								+ '" maxlength="' + field.maxlength + '" value="' + field.defaultVal + '" size="'
								+ field.size + '" min="' + field.min + '" max="' + field.max 
								+ '" class="' + field.fieldClass + '" ' + attrFrag + '/>';
						break;
				}
				return '<td>'+html+'</td>';
			}
